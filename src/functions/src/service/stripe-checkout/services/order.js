...
async function create(checkoutSessionId) {
  ...
  const completedOrder = await airtable.createRecord(process.env.CHECKOUT_ORDER_AIRTABLE_VIEW, orderData)
...
async function addItems(items, orderId) {
  ...
  await airtable.createRecord(process.env.CHECKOUT_ORDER_AIRTABLE_ITEMS_VIEW, items)
}
